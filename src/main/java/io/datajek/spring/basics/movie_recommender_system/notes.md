...
## Mixing Bean Scope
- let us learn what happens when the dependency of a singleton bean has prototype scope and how to get the correct output

**Singleton Bean with prototype dependency**
- here, we will discuss an interesting problem of mixing bean scopes
...
- our basic implementation of the filter compares different movies and assigns a similarity score
- Hence, Movie is a dependency of the ContentBaseFilter class
- the cbf bean ha singleton scope because we need only one instance of the filter
...
- this class has necessary variables and a static member instances to maintain the global count of instances created.
- the variable instances is incremented in the constructor
- the scope of the Movie class is prototype because we want multiple objects of this class in the cbf class.
...
````
- we will mdify the cbf class to create a variable instances for keeping track of the number of objects created.
- This variable is incremented in the constructor.
- we will also create a dependency on the Movie class using the @Autowired annotatio 
...
```
- in the `main()` method, we will get the cbf bean from the applicatio context ad then use it to retrieve the Movie bean thrice
```java
public static void main(String[] args) {
  ApplicationContext appContext = SpringApplication.run(MovieRecommenderSystemApplication.class, args);
...
```
- we expect one instance of cbf bean and three instances of the prototype Movie bean but the output is different, as can be seen by running the above code
- the output of the code shows that the same Movie bean is returned every time
- Morever, the number of instances of the prototype bean created is one instead of three
- a singleton bean is created when the context is loaded
- the Movie constructor was called by Spring when it was creating the cbf bean.
...
- **NB:** when a prototype bean is injected into a singleton bean, it loses its prototype behaviour and acts as a singleton
...
- one of them is by using a procy
...
```
- the prototype bean does not get autowired into the singleton bean at the time of its creation
...
- after making this code change a proxy Movie object is created and we get a new Movie bean when the cbf bean calls the getMovie() method on the proxy object.
...
-  as can be seen from the output, the singleton bean constructor is called when the cbf object is initialized, but the Movie constrictor is not called at that time
- the Movie constructor is caled whenever the proxy object gets used like when it gets printed out
...
- one thing to consider is that singleton scope miimizes the number of objects created so the scope should only be changed where necesary 
